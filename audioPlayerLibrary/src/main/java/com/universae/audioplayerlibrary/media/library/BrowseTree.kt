...
class BrowseTree(
    ...
    init {
        ...
        val recommendedCategory = MediaMetadata.Builder().apply {
            ...
            setArtworkUri(
                Uri.parse(
                    RESOURCE_ROOT_URI +
                            context.resources.getResourceEntryName(R.drawable.ic_auriculares)
                )
            )
        ...
        val albumsMetadata = MediaMetadata.Builder().apply {
            ...
            setArtworkUri(
                Uri.parse(
                    RESOURCE_ROOT_URI +
                            context.resources.getResourceEntryName(R.drawable.ic_tema)
                )
            )
            setIsPlayable(false)
        ...
        musicSource.forEach { mediaItem ->
            ...
            val albumChildren: MutableList<MediaItem> = mediaIdToChildren[albumMediaId] ?: buildAlbumRoot(mediaItem)
     ...
     */
    // operator fun get(mediaId: String) = mediaIdToChildren[mediaId] //TODO: codigo original
    operator fun get(mediaId: String): MutableList<MediaItem>? {
        if (mediaId == "/") {
            // Filtrar los MediaItem con mediaId vacío o nulo
            return mediaIdToChildren[UAMP_BROWSABLE_ROOT]
            //return mediaIdToMediaItem.values.filter { it.mediaId != null && it.mediaId.isNotEmpty() }.toMutableList()
        } else {

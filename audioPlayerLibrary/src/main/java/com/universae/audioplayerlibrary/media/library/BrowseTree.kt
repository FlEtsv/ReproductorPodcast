 ...
 *  ...
 *
 *  Requesting `browseTree["root"]` would return a list that included "Asignaturas", and
...
class BrowseTree(
     ...
     */
    init {
        val rootList = mediaIdToChildren[REPRODUCTOR_BROWSABLE_ROOT] ?: mutableListOf()
        ...
        val albumsMetadata = MediaMetadata.Builder().apply {
            setTitle(context.getString(R.string.temas_titulo))
            setArtworkUri(
                Uri.parse(
                    RESOURCE_ROOT_URI +
                            context.resources.getResourceEntryName(R.drawable.ic_tema) //TODO: Cambiar por icono de asignaturas
                )
            )
            setIsPlayable(false)
        ...
        rootList += MediaItem.Builder().apply {
            setMediaId(TEMAS_ROOT)
            setMediaMetadata(albumsMetadata)
        }.build()

        mediaIdToChildren[REPRODUCTOR_BROWSABLE_ROOT] = rootList
    ...
    private fun buildAlbumRoot(mediaItem: MediaItem): MutableList<MediaItem> {
        ...
        val rootList = mediaIdToChildren[TEMAS_ROOT] ?: mutableListOf()
        ...
        mediaIdToChildren[TEMAS_ROOT] = rootList
